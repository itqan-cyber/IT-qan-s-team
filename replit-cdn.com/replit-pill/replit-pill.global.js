  ...
  (function () {
    ...
    if (
      ...
      document.referrer.startsWith(
        "https://replit-com.web-sandbox.oaiusercontent.com"
      )
    )
      return;
    let m = (() => {
        let l = window.location.origin;
        return !!(l.endsWith(".replit.dev") || l.endsWith(".repl.co"));
      })(),
      s = !0,
      h = document.currentScript?.getAttribute("data-referral-code"),
      C = () => {
        try {
          let l = !1;
          try {
            l =
              window.localStorage?.getItem("replit-pill-preference") ===
              "hidden";
          } catch {}
          if (l || document.getElementById("replit-pill-host")) return;
          let c = document.createElement("div");
          c.id = "replit-pill-host";
          let b = c.attachShadow({ mode: "closed" }),
            y = `
      ...
      `;
          ...
          u.className = "badge-content";
          let M = x("10.67", "16", "0 0 50 75", y, "replit-logo"),
            g = document.createElement("div");
          (g.textContent = "Made with Replit"), (g.className = "replit-text");
          let r = document.createElement("button");
          (r.className = "close-button"), r.setAttribute("aria-label", "Close");
          let a = x("24", "24", "0 0 24 24", E, "close-icon");
          a.setAttribute("stroke", "currentColor"),
            ...
            n.append(v, u, r),
            (n.onclick = (t) => {
              if (t.target !== r && t.target !== r.firstChild) {
                let o = h
                  ? `https://replit.com/referral-code/${h}?source=badge&referrer=${encodeURIComponent(
                      window.location.origin
                    )}`
                  : "https://join.replit.com/badge_v3";
                if (d && m && window.parent) {
                  let i = { type: "CLICKED_MADE_WITH_REPLIT_BADGE" };
            ...
            (r.onclick = (t) => {
              ...
              try {
                window.localStorage.setItem("replit-pill-preference", "hidden");
              } catch {}
            }),
            b.appendChild(w),
            b.appendChild(n),
            ...
            d && m
              ? window.addEventListener("message", (t) => {
                  let o = t.origin,
                    i = new URL(o),
                    f =
                      i.hostname === "replit.com" ||
                      i.hostname.endsWith(".replit.com") ||
                      i.hostname.endsWith(".replit.dev") ||
                      i.hostname.endsWith(".repl.co"),
                    p = !1;
                  if (document.referrer)
                    try {
                      let S = new URL(document.referrer).origin;
                      p = o === S;
                    } catch {}
                  if (!f && !p) return;
                  let e = t.data;
                  if (!(!e || typeof e != "object" || !e.type)) {
                    if (e.type === "SHOW_MADE_WITH_REPLIT_BADGE") {
                      if (s) return;
                      (n.style.display = "inline-flex"), (s = !0);
                    } else if (e.type === "HIDE_MADE_WITH_REPLIT_BADGE") {
                      if (!s) return;
                      (n.style.display = "none"), (s = !1);
                    }
                  }
                })
              : (s = !0);
